...
import dao.IPeriodical;
import entities.Periodical;
import entities.PeriodicalCategory;
import entities.PeriodicalType;
...
public class PeriodicalDAO extends AbstractDAO implements IPeriodical {
    ...
    private static PeriodicalDAO periodicalDAO = null;
    ...
    public Periodical findPeriodicalById(int id) {
        ...
        try (Connection connection = daoFactory.getConnection();
             PreparedStatement statement = connection.prepareStatement(FIND_PERIODICAL_BY_ID)) {
            statement.setInt(1, id);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    periodical = new Periodical(resultSet.getInt("id"),
